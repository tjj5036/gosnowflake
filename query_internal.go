...
const (
	COMPRESSION_TYPE_AUTODETECT = "auto_detect"
...
// help from Snowflake. This should NOT calculate the SHA 256 digest which is used to filter.
func processCompressionOptions(data *execResponse, fm map[string]fileMetadata) {
	InitFileTypes()
	sc := strings.ToLower(data.Data.SourceCompression)

	ad := sc == COMPRESSION_TYPE_AUTODETECT
	var err error = nil
	var usc fileType
	if !ad && sc != COMPRESSION_TYPE_NONE {
		usc, err = GetFileTypeForMimeType(sc)
		if err != nil {
			panic(err)
		}
	}
	glog.V(2).Infof("Using compression type %s", usc.name)
	glog.Flush()

	for k, v := range fm {
		if ad {
			// Guess the mime type as derived from the file name
			ext := filepath.Ext(v.srcFileName)
			m := mime.TypeByExtension(ext)
			typeEnc := strings.Split(m,  "/")
			if len(typeEnc) != 2 {
				panic("Unsupported ext")
			}
			ms := strings.Split(typeEnc[1], ";")[0] // charset
			ft, canCompress := GetFileTypeForMimeType(ms)
		} else {
